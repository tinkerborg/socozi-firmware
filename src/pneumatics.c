...
void pneumatics_lumbar_release(void)
{
    ...
    /* A tap: keep going to the level already stored, or to the ceiling if there
     * is none, which is what the reference inflate does when left alone.
     */
    {
...
#if ENH_LUMBAR_HOLD_SET
        /* Two meanings only. Inflated or inflating, this says stop and empty;
         * anything else, including part way through a deflate the user has
         * changed their mind about, says start.
         */
        if (lumbar_state == LUMBAR_INFLATE || lumbar_state == LUMBAR_HOLD) {
            lumbar_choosing = 0;
            set_lumbar(LUMBAR_DEFLATE);
        } else {
            /* The ceiling until the release says otherwise, so a button that
             * is never released still stops somewhere.
             */
            lumbar_choosing  = 1;
