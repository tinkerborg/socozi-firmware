...
- A tap **SHALL** store whatever level it settles on, including the first-use
  default, so the stored level always matches what is actually in the cell. The
  add-to-it case depends on that being true.
